...
export function ReviewForm({ sampleId, targetText, contrasts }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage("تم حفظ المرجع البشري للمراجع.");
      router.refresh();
    } catch (error) {
  ...
  return (
    <div className="card reviewer-form-card">
      <div className="reviewer-form-heading">
        <ShieldCheck size={22} />
        <div>
          <h2>الحكم البشري المستقل</h2>
          <p>استمع للتسجيل أولًا ثم قيّم ما سمعته. لا تنسخ حكم المشارك أو نتيجة Azure تلقائيًا.</p>
        </div>
      </div>
      ...
      {verdict === "incorrect" && (
        <div>
          ...
          <div className="contrasts">
            {contrasts.map((contrast) => <button type="button" key={contrast} className={`contrast ${observedText === contrast ? "active" : ""}`} onClick={() => setObservedText(contrast)}>{contrast}</button>)}
          </div>
      ...
      <select id="review-quality" className="select" value={quality} onChange={(event) => setQuality(event.target.value)}>
        <option value="good">جيد</option>
        <option value="noisy">ضوضاء</option>
        <option value="too_short">قصير جدًا</option>
        <option value="silence">صمت</option>
        <option value="unclear">غير واضح</option>
        <option value="corrupt">ملف تالف</option>
      </select>

      <label className="label" htmlFor="review-confidence">ثقة المراجع</label>
      <select id="review-confidence" className="select" value={confidence} onChange={(event) => setConfidence(event.target.value)}>
      ...
      <textarea id="review-notes" className="textarea" value={notes} onChange={(event) => setNotes(event.target.value)} />
